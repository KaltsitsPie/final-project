    ...
    for x in X:
        ...
        for j in range(1, len(Y) + 1):
            current.append(min(
                current[j - 1] + delta,
                prev[j] + delta,
                prev[j - 1] + alpha_table[(x, Y[j - 1])]
            ))
        prev = current
    return prev

def div_and_conquer_alignment(s0, t0,delta,alpha_table):
    m = len(s0)
    n = len(t0)
    if m < 2 or n < 2:
        ...
        left_part = div_and_conquer_alignment(s0[:mid], t0[:split_point],delta,alpha_table)
        right_part = div_and_conquer_alignment(s0[mid:], t0[split_point:],delta,alpha_table)
...
aligned_s0, aligned_t0, minimum_cost = div_and_conquer_alignment(generated_s0, generated_t0,delta,alpha_table)
end_time = time.time()
